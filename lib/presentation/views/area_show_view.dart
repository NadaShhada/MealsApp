...
class AreaShowView extends StatefulWidget {
  final String strArea;

  const AreaShowView(this.strArea);

  @override
  State<AreaShowView> createState() => _AreaShowViewState();
}

class _AreaShowViewState extends State<AreaShowView> {
  @override
  Widget build(BuildContext context) {
    SearchBYAreaCubit cubit = SearchBYAreaCubit();
    final _controller = ScrollController();

    return BlocBuilder<SearchBYAreaCubit, ModelsState>(
        builder: (cx, ModelsState state) {
      var bloc = Provider.of<SearchBYAreaCubit>(context, listen: false);

      if (state is ModelsLoadingState) {
        bloc.getMealsByArea(AreaParams(widget.strArea));
          ...
          child: Column(
            children: [
              AppBar(
                leading: BackButton(
                  onPressed: () {
                    bloc.getMealsByAreaLoadingState();
                    Navigator.pop(context, () {
                      setState(() {});
                    });
                  },
                ),
                title: Text(widget.strArea),
              ),
              const SizedBox(
                height: 15,
              ),
